...
function createData(
  ...
  return { idTask, titleTask, descriptionTask, initialTask, stopTask, statusTask, resourceTask };
}
...
//Componente ListeningTask
const ListeningTask = () => {
  const [open, setOpen] = useState(false);
  const [openEdit, setOpenEditar] = useState(false);
  const [tarefas, setTarefas] = useState([]);
  const [tarefa, setTarefa] = useState();
  const [idTaskSelecionada, setidTaskSelecionada] = useState([]);
  ...
  const handleDeletar = (id) => {
            ...
            <Table sx={{ minWidth: 650 }} size="small" aria-label="a dense table">
                ...
                {tarefas.map((row, indice) => (
                    ...
                    >
                      ...
                      <TableCell align="right">{row.initialTask}</TableCell>
                      <TableCell align="right">{row.stopTask}</TableCell>
                      <TableCell align="right">{row.statusTask}</TableCell>
                      <TableCell align="right">{row.resourceTask}</TableCell>
                      <TableCell align="center">
                        <Button variant="contained" color="success" onClick={() => handleEditar(row.idTask)}><EditIcon fontSize="small" /></Button>            
                      </TableCell>
                      <TableCell align="center">
                        <Button variant="contained" color="error" onClick={() => handleDeletar(row.idTask)}><DeleteIcon fontSize="small" /></Button>            
                      </TableCell>
                    </TableRow>
                ))}
                </TableBody>
    ...
    <div>
      ...
      >
        <div>
          <CreateTask handleClose={handleClose} tarefas={tarefas} setTarefas={setTarefas} />
        </div>
      </Modal>  
    </div>
    <div>
      ...
      >
        <div>
          <EditTask handleCloseEditar={handleCloseEditar} idTaskSelecionada={idTaskSelecionada} tarefas={tarefas} tarefa={tarefa} setTarefas={setTarefas} />
        </div>
      </Modal>  
    </div>
  </>    
