...
//Declaração do componente EditTask, recebendo como props, do Componente ListarTarefa, os states handCloseEditar,
// idTarefaSelecionada, tarefas, tarefa e setTarefas
const EditTask = ({handleCloseEditar, idTarefaSelecionada, tarefas, tarefa, setTarefas}) =>{
  const [idTask, setIdTask] = useState();
  const [titleTask, setTituloTarefa] = useState('');
  ...
  useEffect(() => {
    ...
    setIdTask(idTarefaSelecionada);
    setTituloTarefa(tarefa.titleTask);
  ...
  const handleEditar = () => {
    ...
    setTarefas(current =>
      current.map(obj => {
        if (obj.idTask === idTarefaSelecionada) {
          console.log('obj: ' + JSON.stringify(obj));          
          return {...obj, 
              idTask:idTarefaSelecionada,
              titleTask:titleTask,
        ...
        <CardContent sx={{
          ...
          <Grid item xs={12}>
            <FormControl fullWidth>
              <Input id="tarefa_titulo" aria-describedby="tarefa_titulo_helper_text" value={titleTask} onChange={e => { setTituloTarefa(e.target.value) }} />
